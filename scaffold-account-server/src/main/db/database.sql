...
-- 分配权限
grant all privileges on xr_scaffold_db.* to 'xr_scaffold'@'%'identified by '123456' with grant option;
grant all privileges on xr_scaffold_db.* to 'xr_scaffold'@'localhost'identified by '123456' with grant option;
-- 刷新mysql用户权限
flush privileges ;
...
CREATE TABLE `xr_user` (
  ...
  `update_time` int(15) NOT NULL DEFAULT 0 COMMENT '最后更新时间',
  PRIMARY KEY (`user_id`),
